...
public class NotificationOptions {
    ...
    private void instantiateOptions(){
        Option queueName = Option.builder()
                .longOpt("queue_name")
        ...
        Option endpoint = Option.builder()
                .longOpt("notification_queue_endpoint")
                .argName("property=value" )
                .hasArgs()
                .valueSeparator()
                .numberOfArgs(2)
                .desc("The service endpoint in protocol://host:port format. The protocol must be 'grpc' or "
                        + "'grpcs' signifying plaintext or TLS-encrypted communication respectively.")
                .build();
        options.addOption(endpoint);

        Option certificate = Option.builder()
